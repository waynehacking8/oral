...
\begin{tabular}{lcccl}
\toprule
\textbf{Method} & \textbf{Safety (HS $\downarrow$)} & \textbf{Accuracy (FA $\uparrow$)} & \textbf{Efficiency} & \textbf{Overhead vs.\ SFT} \\
\midrule
...
\node[rounded corners=4pt, fill=ntustblue!8, draw=ntustblue!50, text width=13cm, align=center, inner sep=8pt, font=\small] at (0, 0) {
  ...
  Vanilla SFT takes only 3.7 min / 34.8 GB --- at typical poison ratios ($p \leq 0.10$),\\
  most samples are benign, so \textbf{the majority of defense computation is wasted}.
};
\end{tikzpicture}
...
\end{center}
\takeaway{Two complementary signals: high loss catches poisoned samples in mid/late training; early memorization catches them in early epochs. Thresholds $\tau_{\text{high}}, \tau_{\text{low}}$ adapt every 50 steps.}
\end{frame}
