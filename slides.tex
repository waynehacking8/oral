...
\small Well-separated loss distributions. SelGrad achieves best HS on both: 1.20\% and 3.98\% respectively, outperforming SafeGrad.
\end{exampleblock}

\column{0.48\textwidth}
\begin{alertblock}{\small Challenging: Qwen-2 (unique tokenization)}
\small Safety preserved (HS=3.59\%, tied-best), but FA degrades to 74.60\%. Higher loss overlap $\Rightarrow$ more false positives $\Rightarrow$ excessive task suppression. Needs architecture-specific threshold calibration.
\end{alertblock}
\end{columns}
\end{frame}
...
\begin{enumerate}
  ...
  \vspace{8pt}
  \item \textbf{Multi-Modal SelGrad \& Federated Safe Fine-tuning}\\
  {\small Extension to vision-language models (LLaVA, GPT-4V). Adaptation for distributed training without centralizing data.}
\end{enumerate}
\end{frame}
...
\small
\begin{itemize}
  \item SelGrad achieves lowest HS ($-$39.7\% vs SafeGrad) with competitive FA ($-$0.7\%)
