...
{\normalsize Date: 2026\,/\,04\,/\,15 \quad\quad Location: T4-516}
\end{center}
\end{frame}
}
...
\end{columns}
\takeaway{\small\textbf{Fine-tuning inherently weakens safety alignment.} Poisoned data makes it catastrophic: even $p{=}0.05$ (50 samples) causes severe compromise.}
\end{frame}
...
\begin{tabular}{lcccc}
\toprule
\textbf{Method} & \textbf{Safety} & \textbf{Accuracy} & \textbf{Efficiency} & \textbf{Cost} \\
\midrule
\textcolor{dangerred}{SafeGrad} & \textcolor{safegreen}{\checkmark\ Strong} & Moderate & \textcolor{dangerred}{$\times$\ High} & \textcolor{dangerred}{18.0m / 78.3G} \\
\textcolor{dangerred}{AsFT} & \textcolor{safegreen}{\checkmark\ Good} & \textcolor{dangerred}{$\times$\ Degraded} & \textcolor{dangerred}{$\times$\ High} & \textcolor{dangerred}{24.4m / 33.4G} \\
Vaccine, RepNoise & Moderate & N/A & \textcolor{dangerred}{$\times$\ Pre-train} & Weeks \\
\midrule
...
$\mathcal{L}_p \sim \mathcal{N}(5.3,\, 1.2^2)$ \quad (Cohen's $d \approx 3.14$)
